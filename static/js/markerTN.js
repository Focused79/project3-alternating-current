var myMap = myMap || L.map('map').setView([35.5175, -86.5804], 7);
...
// Tennessee bounding box coordinates
const tennesseeBoundingBox = {
    minLatitude: 34.982972,
    maxLatitude: 36.678118,
    minLongitude: -90.310298,
    maxLongitude: -81.6469
};
...
function addMarkersForYear(year) {
    ...
    d3.json(url).then(function(response) {
        ...
        for (let i = 0; i < features.length; i++) {
            ...
            if (!isNaN(latitude) && !isNaN(longitude) && state === "TN" && (!year || openDate === year)) {
                // Check if station is within Vermont bounding box and matches the selected year
                if (latitude >= tennesseeBoundingBox.minLatitude &&
                    latitude <= tennesseeBoundingBox.maxLatitude &&
                    longitude >= tennesseeBoundingBox.minLongitude &&
                    longitude <= tennesseeBoundingBox.maxLongitude) {
